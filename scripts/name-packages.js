...
function namePackages(moduleBaseName, opts = {}) {
  ...
  for (const packagesSortName of packages) {
    ...
    if (!packageJson.version || packagesSortName === packageBaseName) {
      packageJson.version = require(path.join(
        __dirname,
        "../lerna.json"
      )).version;
    }
    ...
    if (
      !packageJson.module &&
      fs.existsSync(
        path.join(packagesRoot, packagesSortName, "tsconfig.esm.json")
      )
    ) {
  ...
  for (const packagesSortName of packages) {
    ...
    const oldReferences =
      tsconfigJson.references || (tsconfigJson.references = []);
