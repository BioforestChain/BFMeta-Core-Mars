...
export class BlobHelper {
  ...
  async open(openArg: BFChainCore.OpenBlobArgJSON) {
    const { blobSha256Reader } = this;
    if (blobSha256Reader === undefined) {
      throw new NoFoundException(ERROR_LIST.PARAM_LOST, {
        param: "blobSha256Reader",
      });
    }
    const { hash, algorithm } = openArg;
    if (algorithm !== "SHA256") {
      throw new ArgumentIllegalException(ERROR_LIST.BLOB_ALGORITHM_IS_INVALID, {
        hash,
        algorithm,
      });
    }
    return await blobSha256Reader.open(hash);
  }

  async read(readArg: BFChainCore.ReadBlobArgJSON) {
    const { blobSha256Reader } = this;
    if (blobSha256Reader === undefined) {
    ...
    const chunkBuffer = await blobSha256Reader.read(readArg.descriptor, readArg.start, readArg.end);
    const readResult: BFChainCore.ReadBlobReturnParams = { chunkBuffer };
    return readResult;
  }

  async close(closeArg: BFChainCore.CloseBlobArgJSON) {
    const { blobSha256Reader } = this;
    if (blobSha256Reader === undefined) {
      throw new ArgumentIllegalException(ERROR_LIST.PARAM_LOST, {
        param: "blobSha256Reader",
    ...
    return await blobSha256Reader.close(closeArg.descriptor);
  }
  ...
  requestStorage(
    ...
    const { blobSha256Writer } = this;
    if (!blobSha256Writer) {
      throw new RefuseException(ERROR_LIST.PARAM_LOST, {
        param: "blobSha256Writer",
      });
    }
    return blobSha256Writer.requestStorage(openArg, totalSize, chunkSize, contentType, strategy);
  }
  ...
  saveChunk(pointer: string, index: number, chunk: Uint8Array) {
    ...
    return blobSha256Writer.saveChunk(pointer, index, chunk);
  }
  ...
  saveAsBlob(pointer: string) {
    ...
    return blobSha256Writer.saveAsBlob(pointer);
  }
  ...
  changeBlobStrategy(
    ...
    return blobSha256Writer.changeBlobStrategy(openArg, strategy);
  }
}
