declare namespace BFChainCore {
  interface BlobReader {
    ...
    state(fd: string): Promise<BlobMetadata>;
    read(fd: string, start: number, end: number): Promise<Uint8Array>;
  ...
  interface BlobWriter {
     ...
     * @param chunk 分片数据
     */
    saveChunk(ptr: string, index: number, chunk: Uint8Array): Promise<void>;
    /**
     * 保存成不可变的 Blob 对象
     * @param ptr 逻辑地址
     ...
     * @returns 返回HASH值
     */
    saveAsBlob(ptr: string): Promise<string>;
