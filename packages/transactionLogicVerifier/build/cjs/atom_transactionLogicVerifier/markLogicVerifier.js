...
const { ConsensusException, NoFoundException } = core_util_exception_1.CoreExceptionGenerator("VERIFIER", "MarkLogicVerifier");
let MarkLogicVerifier = class MarkLogicVerifier extends _txbaseLogicVerifier_1.TransactionLogicVerifier {
    constructor() {
        super();
    }
    async verify(transaction, currentBlockHeight, accountGetterHelper = this.accountGetterHelper, transactionGetterHelper = this.transactionGetterHelper, customTransactionCenter = this.customTransactionCenter) {
        ...
        const sender = this.logicVerify(transaction, currentBlockHeight, accountGetterHelper, transactionGetterHelper);
        const mark = transaction.asset.mark;
        const { sourceChainMagic, dappid } = mark.dapp;
        await this.isDAppidAlreadyExist(sourceChainMagic, dappid, currentBlockHeight, accountGetterHelper);
        return true;
    }
    /**
     * dappid 是否已经存在
     ...
     */
    async isDAppidAlreadyExist(magic, dappid, currentBlockHeight, accountGetterHelper = this.accountGetterHelper) {
        const Function_Exception_Detail = {
...
MarkLogicVerifier = __decorate([
    util_1.Injectable(),
    __metadata("design:paramtypes", [])
], MarkLogicVerifier);
exports.MarkLogicVerifier = MarkLogicVerifier;
//# sourceMappingURL=markLogicVerifier.js.map