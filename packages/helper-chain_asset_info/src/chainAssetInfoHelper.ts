...
export class ChainAssetInfo implements BFChainCore.AssetInfoJSON {
  constructor(public chainName: string, public magic: string, public assetType: string) {}
}
/**链资产类型的缓存 */
...
export class ChainAssetInfoHelper {
  ...
  /**获取链资产类型 */
  getAssetInfo(chainName: string, magic: string, assetType: string) {
    const key = chainName + ":" + magic + ":" + assetType;
    let chainAssetInfo = CHAIN_ASSET_INFO_CACHE.get(key);
    if (!chainAssetInfo) {
      chainAssetInfo = new ChainAssetInfo(chainName, magic, assetType);
