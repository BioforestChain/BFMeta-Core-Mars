 ...
 * 为数据收发处理器包装数据处理
 */
export class ChainChannel extends ChainChannelBase {
  @Inject("bfchain-core:TransactionCore")
  protected transactionCore!: import("../transaction").TransactionCore;
        ...
        try {
          switch (cmd) {
            ...
            case DUPLEX_API_CMD.QUERY_TRANSACTION: {
              /**查询交易的响应，默认为繁忙 */
              const response = QueryTransactionReturnModel.fromObject({
            ...
            case DUPLEX_API_CMD.NEW_TRANSACTION: {
              /**广播交易的响应，默认为繁忙 */
              const response = NewTransactionReturnModel.fromObject({
                status: RESPONSE_STATUS.busy,
              });
            ...
            case DUPLEX_API_CMD.QUERY_BLOCK: {
              /**广播交易的响应，默认为繁忙 */
              const response = QueryBlockReturnModel.fromObject({
                status: RESPONSE_STATUS.busy,
              });
            ...
            case DUPLEX_API_CMD.NEW_BLOCK: {
              /**广播区块的响应，默认为繁忙 */
              const response = NewBlockReturn.fromObject({
                status: RESPONSE_STATUS.busy,
              });
            ...
            case DUPLEX_API_CMD.GET_PEER_INFO: {
              /**节点信息的响应，默认为繁忙 */
              const response = GetPeerInfoReturnModel.fromObject({
                status: RESPONSE_STATUS.busy,
              });
