...
export class MultipleTransactionFactory extends TransactionFactory<MultipleTransaction> {
   ...
   * @param multiple
   */
  init(body: BFChainCore.TxBodyJSON, multiple: BFChainCore.MultipleAssetJSON) {
  ...
  applyTransaction(
    ...
    return wrapTaskList((taskList) => {
      ...
      for (const transaction of transactions) {
        const factory = this.transactionCore.getTransactionFactoryFromType(transaction.type);
        factory.applyTransaction(transaction, eventEmitter, config);
      }
    });
  }
