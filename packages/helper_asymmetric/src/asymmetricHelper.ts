...
export class AsymmetricHelper {
   ...
   * @param hash
   */
  detachedSign(message: Uint8Array, secretKey: Uint8Array) {
  ...
  detachedVeriy(message: Uint8Array, signatureBuffer: Uint8Array, publicKeyBuffer: Uint8Array) {
    const hash = this.cryptoHelper
      .sha256()
      .update(message)
      .digest();
  ...
  asymmetricDecrypt(
    ...
    const decryptedMessage = this.keypairHelper.open(
      encryptedMessage,
      curveEncryptPK,
      curveDecryptSK,
      nonce,
