...
async function getToExchangeAnyTransaction(
  ...
  const data: BFChainCore.TxBodyJSON = {
    ...
    subEnvParams: {},
    type: bfchainCore.transactionHelper.TO_EXCHANGE_ANY, // 交易类型
...
async function getBeExchangeAnyTransaction(
  ...
  const data: BFChainCore.TxBodyJSON = {
    ...
    subEnvParams: {},
    type: bfchainCore.transactionHelper.BE_EXCHANGE_ANY, // 交易类型
    senderId: sender.address, // 发起者地址
    senderPublicKey: sender.publicKey, // 发起者公钥
    senderSecondPublicKey: "", // 发起者二次公钥
    maxFee: "100000000",
  ...
  const beExchangeAny: BFChainCore.BeExchangeAnyJSON = {
    transactionSubId: toExchangeAnyTrs.subId,
  ...
  if (toExchangeAny.cipherPublicKeys.length > 0) {
    ...
    const signature = (
      await bfchainCore.transactionHelper.getCiphertextSignature({
        ...
        transactionSubIdBuffer: parseHexToArrayBuffer(toExchangeAnyTrs.subId),
