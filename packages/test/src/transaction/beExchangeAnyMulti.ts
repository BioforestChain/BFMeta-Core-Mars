...
async function getBeExchangeAnyMultiTransaction(
  ...
  for (const toExchangeAsset of toExchangeAssets) {
    ...
    results.push(toExchangeAsset);
  }

  const beExchangeAnyMulti: BFChainCore.BeExchangeAnyMultiJSON = {
    transactionSubId: toExchangeAnyMultiTrs.subId,
    toExchangeAssets: results,
    beExchangeAsset: {
      ...beExchangeAsset,
      beExchangeAssetPrealnum: assetPrealnum.toString(),
    },
  };
  ...
  if (cipherPublicKeys.length > 0) {
    ...
    const signature = (
      await bfchainCore.transactionHelper.getCiphertextSignature({
        ...
        transactionSubIdBuffer: parseHexToArrayBuffer(toExchangeAnyMultiTrs.subId),
  ...
  console.log(toExchangeAnyMultiTrs.asset.toExchangeAnyMulti.cipherPublicKeys);
