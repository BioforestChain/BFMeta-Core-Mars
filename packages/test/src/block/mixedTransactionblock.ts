...
function getTransferAssetTransaction(
  sender: AccountModel,
  recipientId: any,
  ...
  const trs = bfchainCore.transaction.createTransaction<TransferAssetTransaction>(
    TransferAssetTransactionFactory,
    data,
    {
      transferAsset,
...
function getBeExchangeSpecialAssetTransaction(
  sender: AccountModel,
  ...
  const trs = bfchainCore.transaction.createTransaction<BeExchangeSpecialAssetTransaction>(
    BeExchangeSpecialAssetTransactionFactory,
    data,
    info,
    keypair,
...
function getAccountWithSecret(secret: string) {
  return {
    secret,
    address: getAddressFromSecret(secret),
...
function getTrsInBlock(height: number, statisticsInfo: StatisticsInfo) {
  ...
  const sender1 = getAccountWithSecret(delegatesSecret[1]);
  const sender2 = getAccountWithSecret(delegatesSecret[2]);
  const sender3 = getAccountWithSecret(delegatesSecret[3]);
  const sender4 = getAccountWithSecret(delegatesSecret[4]);
  const sender5 = getAccountWithSecret(delegatesSecret[5]);
  const sender6 = getAccountWithSecret(delegatesSecret[6]);
  const sender7 = getAccountWithSecret(delegatesSecret[7]);
  const sender8 = getAccountWithSecret(delegatesSecret[8]);
  txs[txs.length] = getTransferAssetTransaction(
    sender2,
    sender3.address,
    "FUCK",
    "fucking",
  ...
  txs[txs.length] = getDestoryAssetTransaction(sender4);
  ...
  //   },
  // };
  const data = getToExchangeAssetTransaction(sender6, sender5.address, toExchangeAsset);
  txs[txs.length] = data;
  txs[txs.length] = getBeExchangeAssetTransaction(sender5, sender6, data.trs);
  //#endregion
  const toExchangeAssetSpecialAssetTrs = getToExchangeSpecialAssetTransaction(
    sender8,
    sender7.address,
  );
  txs[txs.length] = getBeExchangeSpecialAssetTransaction(sender7, toExchangeAssetSpecialAssetTrs);
...
async function getCommonBlockAsync(sender: AccountModel) {
  ...
  const blockTrsItems = getTrsInBlock(height, statisticsInfo);
...
(async () => {
  try {
    const sender = getAccountWithSecret(delegatesSecret[9]);
