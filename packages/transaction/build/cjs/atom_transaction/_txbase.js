"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const core_util_exception_1 = require("@bfchain/core-util-exception");
const core_util_exception_errorcode_1 = require("@bfchain/core-util-exception-errorcode");
const core_model_1 = require("@bfchain/core-model");
const util_1 = require("@bfchain/util");
const { ArgumentIllegalException } = core_util_exception_1.CoreExceptionGenerator("CONTROLLER", "_txbase");
// FIXME: 暂时放在这里 @wmc
/**
 * 网络标识符类型
 *
 */
var BNID_TYPE;
(function (BNID_TYPE) {
    /**测试网络 */
    BNID_TYPE["TESTNET"] = "c";
    /**正式网络 */
    BNID_TYPE["MAINNET"] = "b";
})(BNID_TYPE = exports.BNID_TYPE || (exports.BNID_TYPE = {}));
class TransactionFactory {
    /**
     * 从 json 转出 protobuf-message
     * JSON格式一般是进程内部通讯在使用,所以JSON格式默认不校验
     */
    fromJSON(trs, opts) {
        const transaction = this.init(trs, trs.asset);
        if (opts && opts.verify) {
            this.verify(transaction, opts.config);
        }
        return transaction;
    }
    /**
     * 验证主密码的密钥对是否合法
     *
     * @param keypair
     */
    verifyKeypair(keypair) {
        const Function_Exception_Detail = {
            function: "verifyKeypair",
        };
        if (!keypair) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PARAM_LOST, {
                param: "keypair",
                ...Function_Exception_Detail,
            });
        }
        const Keypair_Exception_Detail = {
            target: "keypair",
        };
        if (!keypair.publicKey) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "publicKey",
                ...Keypair_Exception_Detail,
            });
        }
        if (!keypair.secretKey) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                param: "secretKey",
                ...Keypair_Exception_Detail,
            });
        }
    }
    /**
     * 验证二次密码的密钥对是否合法
     *
     * @param keypair
     */
    verifySecondKeypair(keypair) {
        const Function_Exception_Detail = {
            function: "verifySecondKeypair",
        };
        if (!keypair) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PARAM_LOST, {
                param: "keypair",
                ...Function_Exception_Detail,
            });
        }
        const Keypair_Exception_Detail = {
            target: "keypair",
        };
        if (!keypair.publicKey) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "publicKey",
                ...Keypair_Exception_Detail,
            });
        }
        if (!keypair.secretKey) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                param: "secretKey",
                ...Keypair_Exception_Detail,
            });
        }
    }
    /**
     * 创建完交易后的校验
     *
     * @param body
     * @param asset
     */
    verifyTransactionBody(body, asset, config = this.configHelper) {
        const Function_Exception_Detail = {
            function: "verifyTransactionBody",
        };
        if (!body) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PARAM_LOST, {
                param: "body",
                ...Function_Exception_Detail,
            });
        }
        if (!asset) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PARAM_LOST, {
                param: "asset",
                ...Function_Exception_Detail,
            });
        }
        const TransactionBody_Exception_Detail = {
            target: "body",
            ...Function_Exception_Detail,
        };
        const { baseHelper, accountHelper, configHelper } = this;
        if (!baseHelper.isPositiveInteger(body.version)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "version",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!body.type) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "type",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!baseHelper.isValidTransactionType(body.type)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "type",
                type: "transaction type",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!body.senderId) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "senderId",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!accountHelper.isAddress(body.senderId)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "senderId",
                type: "account address",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!body.senderPublicKey) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "senderPublicKey",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!baseHelper.isValidPublicKey(body.senderPublicKey)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "senderPublicKey",
                type: "account publicKey",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (body.senderId !== accountHelper.getAddressFromPublicKeyString(body.senderPublicKey)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.NOT_MATCH, {
                to_compare_prop: "senderId",
                be_compare_prop: "body",
                to_target: "senderPublicKey",
                be_target: "body",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (body.senderSecondPublicKey) {
            if (!baseHelper.isValidPublicKey(body.senderSecondPublicKey)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "senderSecondPublicKey",
                    type: "account publicKey",
                    ...TransactionBody_Exception_Detail,
                });
            }
        }
        if (body.recipientId) {
            if (!accountHelper.isAddress(body.recipientId)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "recipientId",
                    type: "account address",
                    ...TransactionBody_Exception_Detail,
                });
            }
        }
        if (!this.baseHelper.isValidRange(body.rangeType, body.range)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "range",
                type: "transaction range",
                target: "body",
                ...Function_Exception_Detail,
            });
        }
        if (!baseHelper.isNaturalNumber(body.timestamp)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "timestamp",
                type: "natural number",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!baseHelper.isPositiveInteger(body.applyBlockHeight)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "applyBlockHeight",
                type: "positive integer",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (body.numberOfEffectiveBlocks) {
            const numberOfEffectiveBlocks = body.numberOfEffectiveBlocks;
            if (!baseHelper.isPositiveInteger(numberOfEffectiveBlocks)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "numberOfEffectiveBlocks",
                    type: "positive integer",
                    ...TransactionBody_Exception_Detail,
                });
            }
            const { maxApplyAndConfirmedBlockHeightDiff } = configHelper;
            if (numberOfEffectiveBlocks > maxApplyAndConfirmedBlockHeightDiff) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_SHOULD_LTE_FIELD, {
                    prop: "numberOfEffectiveBlocks",
                    target: "body",
                    field: maxApplyAndConfirmedBlockHeightDiff,
                    ...TransactionBody_Exception_Detail,
                });
            }
        }
        // 校验花费手续费
        this.checkTrsBaseFee(body.fee, TransactionBody_Exception_Detail);
        if (!body.fromMagic) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "fromMagic",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!baseHelper.isValidChainMagic(body.fromMagic)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "fromMagic",
                type: "chain magic",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!body.toMagic) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "toMagic",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (!baseHelper.isValidChainMagic(body.toMagic)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "toMagic",
                type: "chain magic",
                ...TransactionBody_Exception_Detail,
            });
        }
        if (body.sourceIP) {
            if (!baseHelper.isIp(body.sourceIP)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "sourceIP",
                    type: "ip",
                    ...TransactionBody_Exception_Detail,
                });
            }
        }
        if (body.dappid) {
            if (!baseHelper.isValidDAppId(body.dappid)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "dappid",
                    type: "dappid",
                    ...TransactionBody_Exception_Detail,
                });
            }
        }
        if (body.lns) {
            if (!baseHelper.isValidLnsName(body.lns, config.chainName)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "lns",
                    type: "location name",
                    ...TransactionBody_Exception_Detail,
                });
            }
        }
    }
    /**
     * 校验基础信息
     *
     * @param transaction
     */
    verifyBaseInfo(transaction, config = this.configHelper) {
        const Function_Exception_Detail = {
            function: "verify",
        };
        if (!transaction) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PARAM_LOST, {
                param: "transaction",
                ...Function_Exception_Detail,
            });
        }
        this.verifyTransactionBody(transaction, transaction.asset, config);
        const Trs_Exception_Detail = {
            target: "transaction",
            ...Function_Exception_Detail,
        };
        const { baseHelper } = this;
        if (!transaction.id) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "id",
                ...Trs_Exception_Detail,
            });
        }
        if (!transaction.signature) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "signature",
                ...Trs_Exception_Detail,
            });
        }
        if (!baseHelper.isValidSignature(transaction.signature)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "signature",
                type: "signature",
                ...Trs_Exception_Detail,
            });
        }
        if (transaction.signSignature) {
            if (!baseHelper.isValidSignature(transaction.signSignature)) {
                throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                    prop: "signSignature",
                    type: "signature",
                    ...Trs_Exception_Detail,
                });
            }
        }
    }
    /**
     * 校验签名
     *
     * @param transaction
     */
    verifySignature(transaction) {
        this.transactionHelper.verifyTransactionSignature(transaction);
    }
    /**
     * 校验交易 remark 大小
     *
     * @param transaction
     */
    verifyRemarkSize(transaction) {
        this.transactionHelper.verifyTransactionRemarkSize(transaction);
    }
    /**
     * 校验完整交易
     *
     * @param transaction
     */
    verify(transaction, config = this.configHelper) {
        this.verifyBaseInfo(transaction, config);
        this.verifyRemarkSize(transaction);
        this.verifySignature(transaction);
    }
    /**
     * 校验金额
     *
     * @param amount
     * @param propName
     * @param Function_Exception_Detail
     */
    checkAssetAmount(amount, propName, Function_Exception_Detail) {
        if (!amount) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PARAM_LOST, {
                param: propName,
                ...Function_Exception_Detail,
            });
        }
        const { baseHelper } = this;
        if (!baseHelper.isValidAssetNumber(amount)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: propName,
                type: "asset number",
                ...Function_Exception_Detail,
            });
        }
        const minAmount = BigInt(0);
        const inputAmount = BigInt(amount);
        if (minAmount >= inputAmount) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_SHOULD_GTE_FIELD, {
                prop: propName,
                field: "0",
                ...Function_Exception_Detail,
            });
        }
    }
    /**
     * 校验交易花费手续费
     *
     * @param fee
     * @param Function_Exception_Detail
     */
    checkTrsBaseFee(fee, Function_Exception_Detail) {
        if (!fee) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: "fee",
                ...Function_Exception_Detail,
            });
        }
        const { baseHelper } = this;
        if (!baseHelper.isValidAssetNumber(fee)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: "fee",
                type: "asset number",
                ...Function_Exception_Detail,
            });
        }
        const inputFee = BigInt(fee);
        const miniUnit = BigInt("0");
        if (miniUnit > inputFee) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_SHOULD_GTE_FIELD, {
                prop: "fee",
                field: "0",
                ...Function_Exception_Detail,
            });
        }
    }
    /**
     * 必须是不限定范围
     *
     * @param body
     * @param Function_Exception_Detail
     */
    emptyRangeType(body, Function_Exception_Detail) {
        if (body.rangeType !== core_model_1.RANGE_TYPE.EMPTY) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.SHOULD_BE, {
                to_compare_prop: "rangeType",
                to_target: "body",
                be_compare_prop: "RANGE_TYPE.EMPTY",
                ...Function_Exception_Detail,
            });
        }
    }
    /**
     * 链名是否合法
     *
     * @param chainName
     * @param propName
     * @param Function_Exception_Detail
     */
    checkChainName(chainName, propName, Function_Exception_Detail) {
        if (!chainName) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: propName,
                ...Function_Exception_Detail,
            });
        }
        if (!this.baseHelper.isValidChainName(chainName)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: propName,
                type: "chain name",
                ...Function_Exception_Detail,
            });
        }
    }
    /**
     * 链名是否合法
     *
     * @param chainMagic
     * @param propName
     * @param Function_Exception_Detail
     */
    checkChainMagic(chainMagic, propName, Function_Exception_Detail) {
        if (!chainMagic) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: propName,
                ...Function_Exception_Detail,
            });
        }
        if (!this.baseHelper.isValidChainMagic(chainMagic)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: propName,
                type: "chain magic",
                ...Function_Exception_Detail,
            });
        }
    }
    /**
     * 链名是否合法
     *
     * @param assetType
     * @param propName
     * @param Function_Exception_Detail
     */
    checkAssetType(assetType, propName, Function_Exception_Detail) {
        if (!assetType) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_REQUIRE, {
                prop: propName,
                ...Function_Exception_Detail,
            });
        }
        if (!this.baseHelper.isValidAssetType(assetType)) {
            throw new ArgumentIllegalException(core_util_exception_errorcode_1.PROP_IS_INVALID, {
                prop: propName,
                type: "asset type",
                ...Function_Exception_Detail,
            });
        }
    }
    /**
     * 交易生效，对账务产生影响
     *
     * @param trs
     * @param event
     */
    applyTransaction(trs, event, config = this.configHelper) {
        const tasks = new util_1.TaskList();
        tasks.next = this._beginDealTransaction(trs, event);
        const assetInfo = this.chainAssetInfoHelper.getAssetInfo(config.magic, config.assetType);
        tasks.next = event.emit("fee", {
            type: "fee",
            transaction: trs,
            applyInfo: {
                address: trs.senderId,
                publicKeyBuffer: trs.senderPublicKeyBuffer,
                assetInfo,
                amount: "-" + trs.fee,
                sourceAmount: trs.fee,
            },
        });
        return tasks.tryToPromise();
    }
    /**
     * 统计交易信息
     *
     * @param trs
     * @param event
     */
    _beginDealTransaction(trs, event) {
        return event.emit("beginDealTransaction", {
            type: "beginDealTransaction",
            transaction: trs,
            applyInfo: undefined,
        });
    }
    _applyTransactionEmitAsset(event, transaction, amount, detail) {
        const tasks = new util_1.TaskList();
        tasks.next = event.emit("asset", {
            type: "asset",
            transaction,
            applyInfo: {
                address: detail.senderId,
                publicKeyBuffer: detail.senderPublicKeyBuffer,
                assetInfo: detail.assetInfo,
                amount: "-" + amount,
                sourceAmount: amount,
            },
        });
        if (detail.recipientId) {
            tasks.next = event.emit("asset", {
                type: "asset",
                transaction,
                applyInfo: {
                    address: detail.recipientId,
                    publicKeyBuffer: detail.recipientPublicKeyBuffer,
                    assetInfo: detail.assetInfo,
                    amount: amount,
                    sourceAmount: amount,
                },
            });
        }
        return tasks.tryToPromise();
    }
}
exports.TransactionFactory = TransactionFactory;
//# sourceMappingURL=_txbase.js.map