...
const { ArgumentIllegalException } = core_util_exception_1.CoreExceptionGenerator("CONTROLLER", "IssueSubchainTransactionFactory");
/**
 * issueSubchain 交易工厂
 *
 */
let IssueSubchainTransactionFactory = class IssueSubchainTransactionFactory extends _txbase_1.TransactionFactory {
    constructor(accountHelper, transactionHelper, baseHelper, configHelper, chainAssetInfoHelper, configMap, moduleMap) {
        super();
     ...
     */
    verifyTransactionBody(body, issueSubchainAsset, config = this.configHelper) {
        ...
        if (bnid !== _txbase_1.BNID_TYPE.TESTNET && bnid !== _txbase_1.BNID_TYPE.MAINNET) {
            throw new ArgumentIllegalException(core_util_exception_1.SHOULD_BE, {
        ...
        if (issueSubchain.chainAssetRewardWeight + issueSubchain.numberOfTransactionRewardWeight ===
            0) {
            throw new ArgumentIllegalException(core_util_exception_1.PROP_SHOULD_GT_FIELD, {
                prop: "issueSubchain.chainAssetRewardWeight + issueSubchain.numberOfTransactionRewardWeight",
                field: 0,
                ...IssueSubchainAsset_Exception_Detail,
            });
        }
        ...
        if (hlen === 1) {
            ...
            for (let i = 0; i < hlen - 1; i += 2) {
                ...
                if (heights[i] >= heights[nextIndex]) {
                    throw new ArgumentIllegalException(core_util_exception_1.PROP_SHOULD_GTE_FIELD, {
                        prop: "height",
                        field: "next height",
                        ...IssueSubchainAsset_Exception_Detail,
                    });
        ...
        const { participationTotalChainAsset, participationTotalFee, participationNumberOfAccount, participationNumberOfTransaction, } = issueSubchain;
        ...
        if (participationTotalChainAsset +
        ...
        // 校验交易POW的难度增长系数
        {
            const { denominator: growthFactorDenominator, numerator: growthFactorNumerator, } = issueSubchain.transactionPowOfWorkConfig.growthFactor;
            ...
            if (!(growthFactorNumerator_BI >= growthFactorDenominator_BI &&
        ...
        // 校验交易POW的参与度占比
        if (!baseHelper.isPositiveFloatContainZero(issueSubchain.transactionPowOfWorkConfig.participationRatio)) {
            throw new ArgumentIllegalException(core_util_exception_1.PROP_IS_INVALID, {
        ...
        if (!subchainConfig) {
            ...
            subchainConfig = new core_helper_1.ConfigHelper(subGenesisBlock, this.configHelper.business);
        }
     ...
     * @param issueSubchain
     */
    init(body, issueSubchain) {
        const transaction = core_model_1.IssueSubchainTransaction.fromObject({
            ...body,
            asset: issueSubchain,
        });
        return transaction;
    ...
    applyTransaction(transaction, eventEmitter, config = this.configHelper) {
        ...
        const { chainName, assetType, magic, bnid, maxTPSPerBlock, blockPerRound, delegates, genesisBlock, } = transaction.asset.issueSubchain;
        // 冻结发起账户
...
IssueSubchainTransactionFactory = __decorate([
    util_1.Injectable(),
...
exports.IssueSubchainTransactionFactory = IssueSubchainTransactionFactory;
//# sourceMappingURL=issueSubchain.js.map